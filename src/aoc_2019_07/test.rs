...
fn all_phases_iterator() {
    let mut aps = AllPhaseSettings::from(PHASES_SINGLE);
    println!("{:?}", aps);
    println!("{:?}", aps.next());
    println!("{:?}", aps.next());
    println!("{:?}", aps.next());
...
fn example_one() {
    ...
    assert_eq!(43210, sig);
    let opt = find_optimal_phase_settings(&prog, PHASES_SINGLE, thruster_signal);
...
fn example_four() {
    ...
    let opt = find_optimal_phase_settings(&prog, PHASES_FEEDBACK, thruster_signal_with_feedback);
